...
const simpleCompareByYThenX = (a, b) => compareByYThenX(a, b, EntityGroupingAccessor);
...
describe("Multi-Axis Search, Insertion, and Deletion (Y-primary, X-secondary)", () => {
    ...
    beforeEach(() => {
        ...
        const entityGroup = [
            // (10, 5)
            new EntityGrouping({ topLeft: new Coordinate(10, 5), length: 100 }, [], null),
            // (20, 5) -> Y-collision with e1
            new EntityGrouping({ topLeft: new Coordinate(20, 5), length: 100 }, [], null),
            // (5, 15)
            new EntityGrouping({ topLeft: new Coordinate(5, 15), length: 100 }, [], null),
            // (15, 15) -> Y-collision with e3
            new EntityGrouping({ topLeft: new Coordinate(15, 15), length: 100 }, [], null),
            // (25, 25)
            new EntityGrouping({ topLeft: new Coordinate(25, 25), length: 100 }, [], null),
        ];
    ...
    describe("insertSorted", () => {
        test("inserts a new element at the beginning", () => {
            // New element (0, 0)
            const eNew = new EntityGrouping({ topLeft: new Coordinate(0, 0), length: 100 }, [], null);
        ...
        test("inserts a new element within a Y-collision group", () => {
            ...
            const eNew = new EntityGrouping({ topLeft: new Coordinate(15, 5), length: 100 }, [], null);
            const index = insertSorted(sortedList, eNew, EntityGroupingAccessor);
            expect(index).toBe(1);
            expect(sortedList).toHaveLength(6);
        ...
        test("inserts a new element at the end", () => {
            // New element (50, 50)
            const eNew = new EntityGrouping({ topLeft: new Coordinate(50, 50), length: 100 }, [], null);
            const index = insertSorted(sortedList, eNew, EntityGroupingAccessor);
    ...
    describe("removeSorted", () => {
        ...
        test("throws or fails silently if grouping is not found", () => {
            ...
            const eMissing = new EntityGrouping({ topLeft: new Coordinate(100, 100), length: 100 }, [], null);
