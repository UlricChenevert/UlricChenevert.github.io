...
export class EntityGrouping {
    ...
    sortedEntities;
    constructor(effectiveArea, entities, worldScope, systems, topLeftGrouping, topCenterGrouping, topRightGrouping, centerLeftGrouping, centerRightGrouping, bottomLeftGrouping, bottomCenterGrouping, bottomRightGrouping) {
...
export const GenerateNewGroupingWithEntity = (entity, oldSquareArea, worldScope, groupingRelativePosition) => {
    ...
    const newGrouping = new EntityGrouping(area, [entity], worldScope, BuildSystemsForNewGrouping(), x0y0, x1y0, x2y0, x0y1, x2y1, x0y2, x1y2, x2y2);
...
export const BuildSystemsForNewGrouping = () => {
    const temp = new Map();
    temp.set("Communicate", new ConversationSystem([]));
    return temp;
};
